...
| タイプ | 説明 | 使用レーン | UIでの選択 |
|--------|------|-----------|-----------|
| single-main | フル画面（メインのみ） | メインのみ | 可 |
| split-h | 左右分割（横並び） | メイン + サブ | 可 |
| split-v | 上下分割（縦積み） | メイン + サブ | 可 |
| pip | ワイプ（小窓付き） | メイン + サブ | 実装済み（UI未対応） |
...
interface Segment {
  ...
  subClipId: string | null   // サブレーンのクリップID
...
| pip | 1920×1080 | 480×270（右下配置、マージン40px） |
...
interface BgmConfig {
  ...
  volume: number      // 音量（0.0〜1.0、デフォルト0.5）
...
video-editor/
├── electron/
│   ├── main.ts              # Electronメインプロセス・FFmpeg処理
│   └── preload.ts           # IPC ブリッジ
├── src/
│   ├── components/
│   │   ├── Header.tsx       # アスペクト比選択
...
│   │   ├── SegmentEditor.tsx # セグメント構成・クリップ割り当て
│   │   ├── SegmentPanel.tsx # セグメントパネル
│   │   ├── Lane.tsx         # レーンコンポーネント
...
│   ├── layout-sketch.md     # レイアウト図
│   └── ooui-context.md      # コンテキストドキュメント
├── package.json
├── vite.config.ts
├── tailwind.config.js
└── tsconfig.json
